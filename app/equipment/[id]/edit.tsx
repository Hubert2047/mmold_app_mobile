...
import { updateDevice } from '@/src/services/devices'
...
export default function EditEquipmentScreen() {
    ...
    const { getDeviceById, fetchDevices, loading, devices } = useDevices()
    ...
    useEffect(() => {
        if (existing) {
            ...
            setGatewayId(existing.gatewayId ?? '')
            setExternalId(existing.externalId ?? '')
            setType(existing.type ?? TYPE_OPTIONS[0])
            setPhase(existing.phase === '三相' ? '三相' : '單相')
            setRatedVoltage(existing.ratedVoltage != null ? String(existing.ratedVoltage) : '')
            setRatedCurrent(existing.ratedCurrent != null ? String(existing.ratedCurrent) : '')
            setRatedPowerKw(existing.ratedPowerKw != null ? String(existing.ratedPowerKw) : '')
            setCtRatio(existing.ctRatio != null ? String(existing.ctRatio) : '')
            setAlertsEnabled(existing.alertsEnabled ?? true)
            setPowerSpike(existing.alertPowerSpikeThreshold != null ? String(existing.alertPowerSpikeThreshold) : '')
            setOvercurrent(existing.alertOvercurrentThreshold != null ? String(existing.alertOvercurrentThreshold) : '')
            setNoDataTimeout(existing.alertNoDataTimeout != null ? String(existing.alertNoDataTimeout) : '')
            setImbalance(existing.alertImbalanceThreshold != null ? String(existing.alertImbalanceThreshold) : '')
        }
    }, [existing])
    ...
    useEffect(() => {
        if (!existing && !loading && devices.length > 0) {
    ...
    useEffect(() => {
        ...
        getGateWays(user.organizationId)
            .then(setGateways)
            .catch(() => {})
    }, [user?.organizationId])
    ...
    async function handleSave() {
        ...
        setSaving(true)
        try {
            await updateDevice(user.organizationId, id, {
                name: name.trim(),
